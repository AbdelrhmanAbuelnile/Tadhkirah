<template>
  ...
  <!-- <head>
    ...
    <div class="flex flex-col justify-center items-center lg:justify-end lg:items-end lg:w-full pt-8 px-4">
      ...
      <div class="text-2xl text-right font-medium py-4 flex flex-col items-center justify-center lg:items-end lg:justify-end gap-4 lg:w-4/6" v-for="ayah in ayahs" :key="ayah.number">

        <p class="flex flex-row-reverse items-cneter justify-between full">{{ ayah.text }} <span class="text-center text-red-500 mr-5">{{ ayah.numberInSurah }}</span></p> 
